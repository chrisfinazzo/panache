...
~~~~~~~~ Note that the warnings indicate that `read_csv` may have had some
difficulty identifying the type of each column. This can be solved by using the
`col_types` argument.
...
{line-numbers=off} ~~~~~~~~ > x <- "foo" > y <- data.frame(a = 1L, b = "a")
~~~~~~~~
