...
description: >
  ...
  the CLI, verify it works, connect your editor, run the core commands (`format`
  and  `lint`), and learn how to configure Panache.
---
...
If you use [Aqua](https://aquaproj.github.io/), install `panache` with:
...
edition) installed. You also need `cargo` on your `PATH` if you want to use the
`panache` command directly after installation.
...
`panache lint` exits with an error code whenever any lint issues are found, so
it plugs into CI/CD pipelines directly:

```bash
panache lint .
```

(The `--check` flag is deprecated and now a no-op, since this is the default
behavior.)
...
2. `.panache.toml`, `panache.toml`, or `.config/panache.toml` in current/parent
   directories
3. `panache/config.toml` in the user configuration directory (`~/.config` on
   Linux, `~/Library/Application Support` on macOS, or `%APPDATA%` on Windows)
