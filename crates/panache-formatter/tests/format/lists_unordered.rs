...
fn sentence_wrap_keeps_task_list_hanging_indent() {
    ...
    // Continuation of a task item's own paragraph aligns at the list content
    // column, not past the `[ ]` checkbox — so it sits at the same column as any
    // later block in the item (matching pandoc), and never reaches the 4-space
